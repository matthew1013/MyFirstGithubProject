...
<link href="/assets/pages/css/task-published-list.css" rel="stylesheet" type="text/css" />
                    ...
                    <div class="portlet light bordered">
                        <div class="portlet-title">
                            <div class="caption font-dark">
                                ...
                                <span class="caption-helper hidden-xs">蓝色为公开项目，灰色为非公开项目</span>
                            </div>
...
<!--haokun added 2017/03/14 end 做了个对应发单人拒绝按钮的弹出框-->
...
<div class="task-item" data-open="{{:open}}"  data-id="{{:id}}"> 
    ...
    <div class="row info">
     	<span class="hidden-xs">任务单号：</span><span class="hidden-xs">{{:id}}</span>
		...
		<div class="right">
			<ul>
				<!-- <li><a href="javascript:;" class="zoom up" title="展开"><i class="glyphicon glyphicon-chevron-left"></i><span>展开</span></a></li> -->
         		<li><a href="/dashboard/Finance/records.jsp?taskId={{:id}}" class="record" target="_blank" title="任务财务记录"><i class="iconfont icon-task-finance"></i></a></li>
          		<li><a href="javascript:;" class="remark" title="备忘录" remark="{{:remark}}"  data-id="{{:id}}"><i class="iconfont icon-note"></i></a></li>
				<li><a href="/dashboard/Task/multiplex.jsp?id={{base64Encode id/}}"  class="multiplex" title="任务复用"><i class="iconfont icon-task-multiplex"></i></a></li>
     			{{if state == "0"||state == "1"}} 
				<li><a href="javascript:;"  class="invite-user" title="邀请用户来接单" data-id="{{:id}}"><i class="iconfont icon-invite"></i></a></li>
     			{{/if}}
			</ul> 
		</div>
 	</div>
	...
	<div class="bottom margin-top-20">
		<div class="user">
			{{if state == 2||state == 3||state == 4||state == 5||state == 6||state == 7||state == 8}}
				{{for receiveDemands}}
					{{if state==1}}
		...
		<div class="row operate">
			...
			<button type="button" class="btn task-attach-add {{if state !== "3"}} hide {{/if}}" data-id="{{:id}}" data-date="{{:expireDate}}">增加附加任务</button>
			<button type="button" class="btn task-complete {{if state !== "3"}} hide {{/if}}" data-id="{{:id}}">关闭任务</button>
			<button type="button" class="btn task-end {{if state !== "0"&&state !== "1"&&state !== "2"}} hide {{/if}}" data-id="{{:id}}">关闭任务</button>
			<button type="button" class="btn task-edit {{if state !== "0"&&state !== "1"}} hide {{/if}}" data-id="{{:id}}">修改任务</button>
			<button type="button" class="btn task-select {{if state !== "0"&&state !== "1"}} hide {{/if}}" data-id="{{:id}}">查看并匹配</button>
			<button type="button" class="btn tooltip-test task-pay {{if state !== "5"&&state !== "6"}} hide {{/if}}" data-id="{{:id}}" data-toggle="tooltip" title="点击后，您可以根据接单人的完成情况进行全额或者部分支付">确认支付</button>  <!--haokun modified 2017/03/14 增加了tooltip，指导用户；在class里增加了tooltip-test类-->
			...
			<!--haokun added 2017/03/14 end 增加了一个拒绝支付的按钮；避免某些情况下用户不敢去点击“支付”这个按钮当他不想支付的时候-->
			<button type="button" class="btn task-pay {{if state !== "7"}} hide {{/if}}" data-id="{{:id}}">查看仲裁</button>
			<button type="button" class="btn task-evaluate {{if state !== "8"}} hide {{/if}}" data-id="{{:id}}">评价</button>
		</div>
	</div>
...
<script src="/assets/pages/scripts/task-published-list.js" type="text/javascript"></script>
